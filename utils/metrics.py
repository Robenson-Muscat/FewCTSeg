...
def compute_per_class_dice(model, val_loader, num_classes,
    ...
    Args:
        ...
        show_progress: bool
        plot: bool, display the bar plot if True
    ...
    Example:
        model.eval()
        dice_array, dice_df, macro, weighted = compute_per_class_dice(
    ...
    if support_pos.any():
        weighted_dice = float(
            np.sum(dice_array[support_pos] * supports[support_pos]) /
            np.sum(supports[support_pos])
        )
    else:
    ...
    if plot:
        ...
        heights = np.nan_to_num(dice_array, nan=0.0)
        ...
        plt.xlabel('Class')
        plt.ylabel('Dice')
        plt.title(f'Per-class Dice — macro: {macro_dice:.4f} — weighted: {weighted_dice:.4f}')
        plt.ylim(0.0, 1.0)

        # annotate bars
        for xi, h, is_finite in zip(x, heights, finite_mask):
            if is_finite:
                plt.text(xi, h + 0.01, f'{h:.3f}',
                         ha='center', va='bottom', fontsize=6, rotation=90)
            else:
                plt.text(xi, 0.01, 'abs',
                         ha='center', va='bottom', fontsize=6, rotation=90)
